...
public class GuardAgainstOutOfRangeForDecimal
{
    ...
    {
        var exception = Assert.Throws<GuardOutOfRangeException>(() => Guard.Against.OutOfRange(3.0, "parameterName", 0.0, 1.0, customMessage));
    ...
    {
        var exception = Assert.Throws<GuardOutOfRangeException>(() => Guard.Against.OutOfRange(3.0, expectedParamName, 0.0, 1.0, customMessage));
        Assert.NotNull(exception);
